...
// scale ≥ 50/(50-|pan|) — 현재 최대 pan(±2%) 기준 scale_min ≥ 1.0417 필요, 여유버퍼를
// 두고 1.05로 설정. 줌 스윕 폭(0.13)은 유지하기 위해 최대값도 1.13→1.18로 같이 올림.
// 패턴 3(중앙 고정)은 pan이 항상 0이라 원래도 안전 — 그대로 유지.
const KB_PATTERNS: [number, number, number, number, number, number][] = [
  [1.05, 1.18,  -2,  2,   0,   0],  // 0: 줌인  + 좌→우 패닝
  [1.18, 1.05,   2, -2,   0,   0],  // 1: 줌아웃 + 우→좌 패닝
  [1.05, 1.18,  -2,  2,  -1.5, 1.5], // 2: 줌인  + 대각선 (좌상→우하)
